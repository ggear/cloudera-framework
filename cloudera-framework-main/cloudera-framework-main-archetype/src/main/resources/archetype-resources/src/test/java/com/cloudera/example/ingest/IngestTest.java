...
public class IngestTest extends LocalClusterDfsMrTest implements ConstantsTest {
  ...
  public IngestTest(String[] sources, String[] destinations, String[] datasets, String[][] subsets,
  ...
  public void testIngest() throws Exception {
    ...
    Assert.assertTrue(getFileSystem().listFiles(new Path(getPathDfs(DIR_DS_MYDATASET_RAW_SOURCE_TEXT_TAB)), true)
        .hasNext());
    Assert.assertTrue(getFileSystem().exists(new Path(getPathDfs(DIR_DS_MYDATASET_RAW_SOURCE_TEXT_COMMA))));
    Assert.assertTrue(getFileSystem().listFiles(new Path(getPathDfs(DIR_DS_MYDATASET_RAW_SOURCE_TEXT_COMMA)), true)
        .hasNext());
  }
