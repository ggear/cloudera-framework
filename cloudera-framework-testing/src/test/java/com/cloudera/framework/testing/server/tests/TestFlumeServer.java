...
public class TestFlumeServer implements TestConstants {
  ...
  public void testFlume() throws InterruptedException, IOException, EventDeliveryException {
    assertEquals(1, flumeServer.crankPipeline(ImmutableMap.of("HDFS_ROOT", dfsServer.getPathUri("/")),
        "flume/flume-conf.properties", Collections.<String, String> emptyMap(), Collections.<String, String> emptyMap(),
        "agent1", "source1", "sink1", new SequenceGeneratorSource(), new HDFSEventSink(), "/tmp/flume-sink1", 1));
    assertTrue(flumeServer.crankPipeline(ImmutableMap.of("HDFS_ROOT", dfsServer.getPathUri("/")),
