...
public class Process extends Driver {
  ...
  public int execute() throws InterruptedException, ExecutionException, IOException, ClassNotFoundException {
    ...
    if ((returnValue = stageDriver
      .run(new String[]{pathRaw + Path.SEPARATOR_CHAR + Constants.DIR_REL_MYDS_CANONICAL, pathStaged})) == SUCCESS) {
      if ((returnValue = partitionDriver
        .run(new String[]{pathStaged + Path.SEPARATOR_CHAR + Constants.DIR_REL_MYDS_CANONICAL, pathPartitioned})) == SUCCESS) {
  ...
  public void reset() {
    ...
    for (RecordCounter counter : Stage.COUNTERS) {
      incrementCounter(Stage.class.getName(), counter, 0);
    }
    for (RecordCounter counter : Partition.COUNTERS) {
      incrementCounter(Partition.class.getName(), counter, 0);
    }
    for (RecordCounter counter : Cleanse.COUNTERS) {
      incrementCounter(Cleanse.class.getName(), counter, 0);
    }
  }
