...
import static com.cloudera.example.stream.Stream.PROPERTY_BATCH_SIZE;
import static com.cloudera.example.stream.Stream.PROPERTY_POLL_MS;
import static com.cloudera.example.stream.Stream.PROPERTY_POLL_TICKS;
import static com.cloudera.example.stream.Stream.PROPERTY_RECORD_TYPE;
...
public class Stream extends TestBase {
  ...
  public final TestMetaData testMetaDataCsvBatch = TestMetaData.getInstance() //
      .parameters( //
          ...
          ImmutableMap.of(//
              com.cloudera.example.stream.Stream.PROPERTY_BATCH_SIZE, "5" //
          ), //
          ImmutableMap.of(//
      ...
      .asserts( //
          ImmutableMap.of( //
              com.cloudera.example.ingest.Stage.class.getCanonicalName(),
              ImmutableMap.of( //
          ...
          ImmutableMap.of( //
              com.cloudera.example.ingest.Partition.class.getCanonicalName(),
              ImmutableMap.of( //
          ...
          ImmutableMap.of( //
              com.cloudera.example.ingest.Process.class.getCanonicalName(),
              ImmutableMap.of( //
  ...
  public void testStream(TestMetaData testMetaData) throws IOException, EventDeliveryException, InterruptedException {
    assertEquals(((Integer) testMetaData.getParameters()[2].get(KEY_FLUME_PROCESS_FILE_COUNT)).intValue(),
        flumeServer.crankPipeline(FLUME_SUBSTITUTIONS, FLUME_CONFIG_FILE, testMetaData.getParameters()[0],
            ...
            (String) testMetaData.getParameters()[2].get(KEY_FLUME_SINK_NAME), new com.cloudera.example.stream.Stream(),
    ...
    Driver driverStage = new com.cloudera.example.ingest.Stage(dfsServer.getConf());
    ...
    Driver driverPartition = new com.cloudera.example.ingest.Partition(dfsServer.getConf());
    ...
    Driver driverProcess = new com.cloudera.example.ingest.Process(dfsServer.getConf());
    assertEquals(Driver.RETURN_SUCCESS,
